...
describe("Assign Brands by BO", () => {
    ...
    before(() =>{
      ...
      cy.get('span.MuiChip-label.MuiChip-labelMedium.css-9iedg7').contains('Your Brands Assigned').scrollIntoView()
      .should('be.visible').then(($assignedTag) => {
         if ($assignedTag.length) {
          cy.log("✅ 'Your Brands Assigned' tag found.");
      ...
      cy.get('div.flex.flex-row.items-center.gap-1 > a').each(($el) => {
        ...
        cy.log('📌 Extracted event list:', JSON.stringify(extractedEvent));
        const index = extractedEvent.indexOf(eventName); // Find event index
        if (index !== -1) {
          cy.log(`✅ Event '${eventName}' found in the list.`);
          cy.get(`a:contains("${eventName}")`).should('be.visible').click({ force: true });
                // Step 5: Click on "Brand Owner"
          cy.contains('div', 'Brand Owner').scrollIntoView().should('be.visible').click({ force: true });     
        } else {
          cy.log(`❌ Event '${eventName}' not found in the list.`);
        }
      });
        } else {
      cy.log("❌ 'Your Brands Assigned' tag NOT found on the page. Skipping event selection.");
       }
      });
    })
    ...
    it("CardView Assign Brand", () => {
      ...
      extractBrands().then(() => {
          ...
          // Step 2: Assign brands after extraction
          cy.wrap(extractedBrands).then((brands) => {
            ...
            Object.values(assignBrandData.assigncardbrand).forEach((brandName) => {
              ...
              if (index !== -1) {
                ...
                cy.get('#UnassginedCard-FlatList').then(($btn) => {
                  if ($btn.length > 0 && $btn.is(':visible')) {
                    cy.wrap($btn).scrollIntoView().click();
                    cy.wait(10000);
                    cy.get(extractedBrands).then(($newBrands)=>{
                      expect($newBrands.length).to.be.at.least(extractedBrands.length);
                      extractBrands(); 
                    }); 
                  } else {
                    cy.log('Brand not found!').then(() => {
                      throw new Error(`Brand not found in the list.`);
                    });
                  }
                });
              }
          ...
          // Step 3: Verify Assigned Brands in Search
